...
export const getAllCategoryModel = async () => {
    try {
      const query = "SELECT category_id,category_name,category_url FROM categories";
  ...
  export const getCategoryByIdModel = async (categoryId) => {
    try {
      // Modify the query to join the products table and fetch category along with product details
      const query = `
        SELECT 
          product.product_id, 
